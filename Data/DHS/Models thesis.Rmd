...
model1 <- glm(school_status ~ educ_head + gender + resi + region , data = final_data, family = "binomial")
...
model2 <- glm(school_status ~ wealth + gender + resi + region , data = final_data, family = "binomial")
...
```
```{r}
model3 <- glm(school_status ~ mom_marital + gender + resi + region , data = final_data, family = "binomial")
...
```
```{r}
model4 <- glm(school_status ~ dad_marital + gender + resi + region , data = final_data, family = "binomial")
...
stargazer(model1, model2,model3, model4, coef = list(OR.vector1, OR.vector2, OR.vector3, OR.vector4), 
          title = "Table 3.  estimates early first childbirth and independent variables",
          ci = T,
