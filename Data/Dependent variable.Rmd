...
data2010 <- data2010_raw %>%
  rename(id = llave_ID_lb, wave = ola, age = edad, inedu = estudia, educ_dad = educ_padre, educ_mom = educ_madre, gender = sexo, income = vr_salario, ocupation = ocupacion)

data2013 <- data2013_raw %>% 
   rename(id = llave_ID_lb, wave = ola, age = edad, inedu = estudia, educ_dad = educ_padre, educ_mom = educ_madre, gender = sexo, drop_edu = dejo_estudiar, income = vr_salario, ocupation = ocupacion)
...
freq_income <- income2010 %>% 
  count(income) %>%
  mutate("%" = n / sum(n) *100) %>%
  kable(caption = "Percentage of observations by category in income variable", 
        align = "c") %>% kable_styling(full_width = F)
...
freq_ocu <- ocupation2010 %>% 
  count(ocupation) %>%
  mutate("%" = n / sum(n) *100) %>%
  kable(caption = "Percentage of observations by category in ocupation variable", 
        align = "c") %>% kable_styling(full_width = F)
...
parent_ocupation1 <- merge(wave1, ocupation2010, by= "id", all.x = TRUE)
...
```{r}
#Distribution of variable ocupation analytical sample depv inedu
...
```{r}
table(data01$inedu, parent_educ$educ_dad)
```
